...
public class StudentService {
	...
	// * 인터페이스는 객체 생성 불가, 하지만 부모 참조변수의 역할은 가능하다.
	...
	// 다이아몬드 연산자 : 컴파일러가 좌변 부분을 보고 우변의 타입을 추론하는 것
	...
	public void ex() {	//List 테스트용 메서드
			...
			testList.add(100);	// AutoBoxing (자동으로 기본 자료형을 객체 형태로 변환함)
	...
	public void displayMenu() {
		...
		do {
			try {
				...
				sc.nextLine();	// 입력벼퍼 남아있는 잘못된 코드 + 개행문자 제거
				
				menuNum = -1;	// 첫 반복 시 잘못 입력하는 경우
				// menuNum이 초기값인 0을 가지고 있어 반복문이 종료됨.
	...
	public String addStudent() {
		// CheckedException : 반드시 throws 구문이나 try-catch 처리가 강제된다.
		// IOException, SQLException..
		
		// UncheckedException : 강제 처리 X, 예외 발생하면 자동으로 상위 호출 메서드로 예외가 전파됨.
		// throws InputMismatchException 사용하지 않아도 알아서 전파됨.
...
//		} else {}
	...
	public void selectAll() {
		...
		// boolean List.isEmpty() : 컬렉션이 비어있다면 true 반환, 아니면 false
...
//		}
	...
	public String updateStudent() {
		...
		// E List.set(int index, E e) : 주어진 index번째에 주어진 객체로 덮어쓰기(수정함)
		// 기존의 객체를 반환함
	 ...
	 * 4. 학생 정보 제거 메서드
	 * 
	 * 수정과 같은 검사 후 삭제 시 "정말 삭제하시겠습니까? (Y/N) : "
	...
	public String removeStudent() {
		...
		// 이 때 제거된 요소가 반환됨.
		// * List 중간에 비어있는 요소가 없게 하기 위해서
		// remove() 동작 시 뒤쪽 요소를 한 칸씩 자동으로 당겨오는 행위를 함.
		if(ch == 'Y') {
