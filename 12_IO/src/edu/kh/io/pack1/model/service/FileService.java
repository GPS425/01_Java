...
public class FileService {
	...
	public void method1() {
		...
		// C:/io_test/20251014 폴더 생성
		
		// 제일 앞에 "/" 시작의 의미 : 현재 실행중인 프로젝트의 루트 디렉토리(C)
		// (== 최상위 폴더 : root, 절대 경로의 기준점)
		// 즉, 드라이브를 의미함.
		
		// 명시적으로 맨 앞에 C: , D: 라고 작성 시 해당 드라이브에 폴더 생성됨 >> 절대경로 작성법
		
		// 상대경로 작성법 :
		// io_test/20251014 이렇게 작성 시 >> 해당 프로젝트가 현재 위치!!
		// 해당 프로젝트 안에 io_test/20251014 생성됨
		// 현재 프로젝트 : 12_IO
		//                   ㄴ io.test
		//                          ㄴ 20251014
	...
	/**
	 * File 객체를 이용해서 지정된 위치에 파일을 생성하기
	 */
	public void method2() {
		
		// 문자열에서 \ 하나는 Escape 문자로 사용됨
		// >> \ 자체를 문자로 사용하고 싶을 땐 \\와 같이 2개로 작성(근데 이건 좀 불편함 쓰지마셈)
		File file = new File("\\io_test\\20251014\\파일생성.txt");
		
		// File 클래스의 메서드는 대부분 IOException 발생시킴
		// >> 반드시 예외처리를 해야 하는 Checked Exception
		// >> 예외 처리 필수
	 ...
	 */
	public void method3() {
		File dir = new File("\\workspace\\01_Java\\12_IO");
		...
		for(File f : files) {
			...
			// 매개변수 long 값을 지정된 패턴 형식으로 변환하여 문자열 형태로 반환하는 메서드
			...
			if (f.isDirectory()) {
				size = "";
				// 폴더 크기를 구하는 메서드는 존재 x >> 구하려면 폴더 안의 모든 파일을 순회하여 각 파일 크기 합산하여 사용
			}
			
			String result = String.format("%-15s %-20s %-5s %10s", Filename, date, type, size);
